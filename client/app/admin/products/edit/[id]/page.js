...
import { ArrowLeft, ArrowRight, Save, Plus, X, ImagePlus, Tag, DollarSign, CheckCircle, ChevronRight, LayoutGrid, ImageIcon, ListChecks, AlertCircle } from 'lucide-react';
...
export default function EditProduct() {
    ...
    const [selectedImages, setSelectedImages] = useState([]); // File objects (new uploads)
    const [imagePreviews, setImagePreviews] = useState([]);   // Data URLs for new uploads
    ...
    const handleImageSelect = (e) => {
        ...
        newSelected.forEach(file => {
            ...
            reader.onload = (ev) => {
                ...
                if (previews.length === newSelected.length) {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            const data = await res.json();
            if (!res.ok) {
                setError(data.msg || 'Failed to update product.');
            } else {
                setSuccess(true);
    ...
    if (fetching) {
        return (
            <div className="flex flex-col items-center justify-center min-h-[400px]">
                <div className="w-12 h-12 rounded-full border-2 border-gray-100 border-t-gray-900 animate-spin mb-4" />
                <p className="text-[10px] font-bold uppercase tracking-widest text-gray-400">Loading Product Identity...</p>
            </div>
        );
    }
        ...
        <div className="max-w-4xl mx-auto pb-20">
            ...
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6 mb-12">
                <div className="flex items-center gap-4">
                    ...
                    <div>
                        <h1 className="text-3xl font-extrabold text-gray-900 tracking-tighter">Edit Product</h1>
                        <p className="text-gray-500 font-medium text-sm">Update product details in your catalog.</p>
                    </div>
                </div>
            ...
            {success && (
                <div className="flex items-center gap-4 bg-gray-900 text-white rounded-[2rem] p-6 mb-10 shadow-xl animate-in fade-in slide-in-from-top-4 duration-500">
                    ...
                    <div>
                        ...
                        <p className="text-gray-400 text-sm font-light">Changes saved successfully. Redirecting...</p>
                    </div>
                </div>
            )}
                ...
                {step === 2 && (
                    <div className="space-y-8 animate-in fade-in slide-in-from-right-4 duration-300">
                        <div className="flex flex-col items-center text-center mb-10">
                            ...
                            <p className="text-gray-500 text-sm font-light mt-2">Manage existing images or upload new ones (Max 5 total).</p>
                        </div>

                        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-4">
                            {/* Existing Images */}
                            {existingImages.map((url, index) => (
                                <div key={`existing-${index}`} className="relative aspect-[4/5] rounded-3xl overflow-hidden bg-gray-50 group border border-gray-100">
                                    <img src={url} alt="" className="w-full h-full object-cover" />
                                    <div className="absolute top-2 left-2 px-2 py-0.5 bg-black/50 backdrop-blur-sm text-[8px] text-white font-bold rounded-full uppercase tracking-widest">Live</div>
                                    <button 
                                        type="button" 
                            ...
                            {imagePreviews.map((preview, index) => (
                                <div key={`new-${index}`} className="relative aspect-[4/5] rounded-3xl overflow-hidden bg-gray-50 group border border-gray-100">
                                    <img src={preview} alt="" className="w-full h-full object-cover" />
                                    <div className="absolute top-2 left-2 px-2 py-0.5 bg-gray-100 text-[8px] text-gray-900 font-bold rounded-full uppercase tracking-widest">New</div>
                                    <button 
                                        type="button" 
                ...
                <div className="mt-16 flex items-center justify-between pt-10 border-t border-gray-50">
                    ...
                    <div className="flex gap-4">
                        {step < 3 ? (
                            ...
                            >
                                <Save size={18} /> {loading ? 'Saving Changes...' : 'Update Product'}
                            </button>
                        )}
                    </div>
                </div>
